...
class _UploadPostScreenState extends State<UploadPostScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadUserData() async {
    ...
    if (user != null) {
      try {
        // Direct Firestore query to studentprofile collection
        ...
        if (userDoc.exists) {
          ...
          // Show welcome dialog now that we have the data
          _showWelcomeDialog();
        } else {
  ...
  Future<void> _uploadPost() async {
    ...
    if (user != null) {
      ...
      try {
        ...
        // Get user profile data directly from studentprofile to ensure we have the latest
        DocumentSnapshot userDoc = await FirebaseFirestore.instance
            .collection('studentprofile')
            .doc(user.uid)
            .get();

        // Use data from studentprofile if available
        String authorName;
        String photoURL;
        String department = '';
        String college = '';
        ...
        if (userDoc.exists) {
          ...
          authorName = _userName.isNotEmpty ? _userName : 'Anonymous';
